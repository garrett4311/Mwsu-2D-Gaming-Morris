var destroyer = {
	create: function () {
		...
		//game.physics.enable([this.player1, this.player2], Phaser.Physics.ARCADE);
		// this.player1.body.enable = true;
		// this.player2.body.enable = true;
		// this.player1.body.bounce.x = 0.95;
		// this.player2.body.bounce.x = 0.95;
		...
		// if (name == 'red')
		// {
		// 	this.obstacle.animations.add('spin', 30, true);
		// 	this.obstacle.animations.play('spin');
		// }
		// if (name == 'blue')
		// {
		// 	this.obstacle.animations.add('spin1', 30, true);
		// 	this.obstacle.animations.play('spin1');
		// }
		// if (name == 'rock')
		// {
		// 	this.obstacle.animations.add('spin2', 30, true);
		// 	this.obstacle.animations.play('spin2');
		// }
	...
	update: function () {
			...
			game.physics.arcade.overlap(this.player1.ship, this.player2.ship, this.bumpPlayers, null, this);

			game.physics.arcade.collide(this.player1.ship, this.player2.ship);
			...
			if (frame_counter % spawn_rate == 0) {
				this.spawnObstacle(game.rnd.integerInRange(32, game.width), game.height, speed = obstacle_speed, 0.5, 0.5)
			}
	...
	spawnObstacle: function (x, y, speed, x_scale, y_scale) {
		...
		//var choice2 = game.rnd.integerInRange(0, game.globals.obstacle_icons2.length - 1);
		var name = game.globals.obstacle_icons[choice];
		//var name2 = game.globals.obstacle_icons2[choice2];

		//create the obstacle with its randomly chosen name
		console.log(name)
		var obstacle = this.obstacles.create(x, y, name);
		//this.obstacle.animations.add("fire", null, 60, true);
		
			// if (name == 'red')
			// {
			// 	var fire = this.game.add.sprite(x, y, "flame")
			// 	//this.fire.anchor.setTo(0.5,0.5);
			// 	this.fire.animations.add('flame', 30, true);
		...
		// obstacle2.body.colliderWorldBounds = true
		// obstacle2.body.immovable = true
		// obstacle2.anchor.setTo(.5, .5)
		// obstacle2.scale.setTo(x_scale, y_scale)
		// obstacle2.body.setSize(obstacle2.width + 20, obstacle2.height - 20);
		// obstacle2.body.velocity.y = -speed

		// obstacle2.checkWorldBounds = true;

		// // Kill obstacle/enemy if vertically out of bounds
		// obstacle2.events.onOutOfBounds.add(this.killObstacle, this);
	...
	bumpPlayers: function ()
	{	
		
		if (this.player1.ship.x > this.player2.ship.x)
		{	
			console.log("bump1")
			this.player1.ship.x += 10;
			this.player2.ship.x -= 10;
		}
		else 
		{
			console.log("bump2")
